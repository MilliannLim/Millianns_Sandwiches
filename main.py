...
def view_menu(L):
    for i in range(0, len(L)):
        if len(L[i]) != 2:
            ...
            return None
        output = "{:<3} : {:10} --- ${:}".format(i + 1, L[i][0], L[i][1])
        print(output)
...
def add_to_order(L, O):
    ...
    sandwich_index = validate_integer("Choose index number "
                                      "of sandwiches to order:", 1, len(L))
    sandwich_index -= 1
    ...
    if index != -1:
        ...
        print(output_message)
        temp = [L[sandwich_index][0], L[sandwich_index][1], new_amount]
        if new_amount != 0:
            O.append(temp)
...
def review_order(O):
    ...
    if len(O) != 0:  # so that customer can see what they ordered
        ...
        print(
            "Sorry, you haven't ordered anything yet")  # this is just incase if the user hasn't ordered anything yet because it would come up blank anyway
        return None
...
def full_review(O, C, delivery_charge=4):
    ...
    if len(C) != 0:
        ...
        print("You don't have any delivery details yet")
        return None
...
def take_away(O):
    ...
    review_order(O)
    sandwich_index = validate_integer("Choose which sandwich you would like to remove:", 1, len(O))
    sandwich_index -= 1
    print()
    number = validate_integer("How many would would you like to remove? "
                              "(please use numbers):", 0, O[sandwich_index][2])
